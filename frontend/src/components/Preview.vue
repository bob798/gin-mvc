...
import { renderMarkdown } from '../composables/useRenderer'
...
const htmlSrcdoc = computed(() => {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><base target="_blank">${style}</head><body>${props.source || ''}</body></html>`
})
</script>
